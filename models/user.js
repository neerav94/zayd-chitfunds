...
module.exports.getUserTransactions = function(number) {
    return new Promise((resolve, reject) => {
        database.connection.query("SELECT * FROM payments t1 JOIN subscribers t2 ON t1.group_id = t2.group_id AND t1.token = t2.token WHERE number=?", [number], (error, results, fields) => {
            if(error) {
                reject(error)
            } else {
                resolve(results)
